...
import com.app.quote.app.quotesapp.Client.adapters.ImagesAdapter
...
class GalleryFragment : Fragment(), Adapter_Items_Click {
    ...
    override fun OnClicked(Quote: Qoutes_firebase, position: Int) {
        GlideApp.with(activity!!).load(storage_instance.getReference(Quote.picture)).placeholder(R.drawable.image).into(image)
        var isadded=dbhelper.GetIsAdded(Quote.id)
        dialog_fav.Save(Quote,isadded,position)
        when(isadded){
            false->{ GlideApp.with(activity!!).load(R.drawable.ic_favorite_border_black_24dp).placeholder(R.drawable.image).into(dialog_fav)}
            true->{ GlideApp.with(activity!!).load(R.drawable.ic_favorite_black_24dp).placeholder(R.drawable.image).into(dialog_fav)}
        }
